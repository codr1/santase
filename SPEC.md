...
- `getViewerMatchState(matchState, viewerIndex)`: Returns a `ViewerMatchState` with the opponent's hand replaced by `{count}`, stock replaced by `{count}`, and `declare66GracePeriodMs` included; the opponent's round score is replaced by NaN while the round is in progress (`roundResult` is null) and revealed when the round ends (`roundResult` is present)
